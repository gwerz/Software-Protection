...
void CMy111Dlg::OnOK() 
{
	...
	GetDlgItem(IDC_OUT)->SetWindowText("发现加密锁!");
...
void CMy111Dlg::OnExec() 
{
	...
	CopyFile("1.exe","src.dll",NULL);
	DestFile.Open("src.dll",CFile::modeWrite);
	DestFile.Seek(0x400,CFile::begin);
	DestFile.Write(Buf+0x200,0x200);
	DestFile.Close();

	CreateProcess(NULL,"src.dll",NULL,NULL,FALSE,0,NULL,NULL,&si,&pi);
}
...
BOOL CMy111Dlg::OnDeviceChange(UINT nEventType, DWORD dwData)
{
	if(iSON)
	{
		int nRet=NTFindFirst((LPTSTR)(LPCTSTR)loginword);
		if(0!=nRet) 
		{
